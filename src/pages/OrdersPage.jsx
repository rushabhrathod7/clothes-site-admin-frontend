...
const OrdersPage = () => {
    ...
    const fetchProductDetails = async (productId) => {
        ...
        const actualId = productId && typeof productId === 'object' && productId._id ? productId._id : productId;
        
        if (!actualId || productDetails[actualId]) return;
        
        setLoadingProducts(prev => ({ ...prev, [actualId]: true }));
        try {
            const response = await api.get(`/api/admin/products/${actualId}`);
                                                        ...
                                                        {item.image ? (
                                                            ...
                                                            <div className="w-full h-full bg-muted-foreground/10 flex flex-col items-center justify-center p-2 text-center">
                                                                ...
                                                                <span className="text-xs text-muted-foreground">
                                                                    {item.productExists === false ? 'Product Deleted' : 'No Image'}
                                                                </span>
                                                            </div>
                                                        )}
                                                    </div>
