  ...
  const fetchSalesData = async () => {
    try {
      const response = await api.get(
        `/admin/analytics/sales?period=${timeRange}`
      );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              ...
              <span
                className={`ml-2 ${
                  dashboardStats?.onlineRevenueChange >= 0
                    ? "text-green-500"
                    : "text-red-500"
                }`}
              >
                ({dashboardStats?.onlineRevenueChange >= 0 ? "+" : ""}
                {dashboardStats?.onlineRevenueChange}%)
              </span>
            </div>
            <div className="text-xs text-muted-foreground">
              ...
              <span
                className={`ml-2 ${
                  dashboardStats?.offlineRevenueChange >= 0
                    ? "text-green-500"
                    : "text-red-500"
                }`}
              >
                ({dashboardStats?.offlineRevenueChange >= 0 ? "+" : ""}
                {dashboardStats?.offlineRevenueChange}%)
              </span>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              ...
              <span
                className={`ml-2 ${
                  dashboardStats?.onlineOrdersChange >= 0
                    ? "text-green-500"
                    : "text-red-500"
                }`}
              >
                ({dashboardStats?.onlineOrdersChange >= 0 ? "+" : ""}
                {dashboardStats?.onlineOrdersChange}%)
              </span>
            </div>
            <div className="text-xs text-muted-foreground">
              ...
              <span
                className={`ml-2 ${
                  dashboardStats?.offlineOrdersChange >= 0
                    ? "text-green-500"
                    : "text-red-500"
                }`}
              >
                ({dashboardStats?.offlineOrdersChange >= 0 ? "+" : ""}
                {dashboardStats?.offlineOrdersChange}%)
              </span>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Number(dashboardStats?.totalUsers || 0)}
            </div>
            <p
              className={`text-xs ${
            ...
            >
              {dashboardStats?.usersChange >= 0 ? "+" : ""}
              {dashboardStats?.usersChange}% from last month
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Number(dashboardStats?.totalProducts || 0)}
            </div>
                ...
                <LineChart data={salesData}>
                  ...
                  <Tooltip
                    formatter={(value, name) => {
                      if (name === "onlineTotal")
                        return [`₹${value}`, "Online Sales"];
                      if (name === "offlineTotal")
                        return [`₹${value}`, "Offline Sales"];
